...
- X 的 `searchPages` 尚未迁入；其实现没有给出分页、查询转义和作用域兼容性的完整证据。重新设计由 [Page 正文范围搜索](page_content_search.md) 独立跟踪。
...
2. 依据 [Page 正文范围搜索](page_content_search.md) 的结论判断 X `searchPages` 应标记为部分覆盖、待设计或明确拒绝；本 TODO 不再重复设计搜索范围。
...
### 2026-08-04：拆出 Page 正文 Search 独立设计

- 脱敏证据：复核 X `searchPages` 基线、当前 MCP 工具与 Graph OneNote v1.0 Page 查询/内容读取边界；未执行真实账号请求。
- 结论：对象元数据 Query 与 Page 正文 Search 不能共用能力结论；Search 的范围、分页、候选硬限制和内容处理改由独立 TODO 跟踪。
- 下一步：待 Search 设计与 Mock 完成后，只在本功能盘点中记录 X 能力的最终覆盖状态。
